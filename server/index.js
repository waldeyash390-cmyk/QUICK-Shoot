  ...
  socket.on("message", (raw) => {
    ...
    switch (msg.type) {
      ...
      case "host": {
        ...
        send(socket, { type: "hosted", code, peerId: socket.peerId });
        break;
      }
      ...
      case "join": {
        ...
        const existingPeerIds = room.peerList();
        room.addPeer(socket.peerId, socket);
        socket.roomCode = code;

        // Tell the joiner: your peerId + all existing peers in the room
        send(socket, { type: "joined", code, peerId: socket.peerId, peers: existingPeerIds });

        // Tell all existing peers that a new user arrived
        room.broadcast(socket.peerId, { type: "peer-joined", peerId: socket.peerId });
        break;
      }
      ...
      case "leave": {
        const room = rooms.get(socket.roomCode);
        if (room) {
          room.broadcast(socket.peerId, { type: "peer-left", peerId: socket.peerId });
