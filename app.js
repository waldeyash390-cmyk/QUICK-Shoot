...
async function onChatData(rawArrayBuffer, fromPeerId) {
  ...
  if (msg.t === "msg") {
    ...
    callHandleInvite(fromPeerId, shortId);
...
function callShowNotification(peerShortId) {
  els.callNotifSub.textContent = `${peerShortId} is calling…`;
...
function callHandleInvite(fromPeerId, shortId) {
  if (callIsActive()) {
